...
def process_election_data(election_data, election_name, election_date, region_unit, cur_date='240801'):
    ...
    """
        try:
            ...
            # 지역단위가 시군구이고, 변환코드가 없으면 시군구코드 기반 Fallback 경로 사용
            if (region_unit == "시군구") and (not os.path.exists(mapping_file)):
                logging.warning("법정동 변환코드 없음: 시군구 Fallback 경로로 처리합니다")
                # 지역코드(시군구코드)로 시도명/시군구명 매핑
                try:
                    code_bdong = pdr.code_bdong()[["시도명", "시군구명", "시군구코드"]].drop_duplicates("시군구코드")
                    raw_data['지역코드'] = raw_data['지역코드'].astype(str).str.zfill(5)
                    logging.info("[Fallback] raw_data columns: %s", list(raw_data.columns))
                    logging.info("[Fallback] code_bdong columns: %s", list(code_bdong.columns))
                    # 키 컬럼 타입/예시 로깅
                    logging.info("[Fallback] raw 지역코드 dtype: %s, sample: %s", raw_data['지역코드'].dtype, raw_data['지역코드'].head(5).tolist())
                    logging.info("[Fallback] map 시군구코드 dtype: %s, sample: %s", code_bdong['시군구코드'].dtype, code_bdong['시군구코드'].head(5).tolist())

                    merged_admin = raw_data.merge(
                        code_bdong,
                        how='left',
                        left_on='지역코드',
                        right_on='시군구코드',
                    ...
                    # 시도명/시군구명 보정 (중복 컬럼 대비)
                    if '시도명' not in merged_admin.columns and '시도명_map' in merged_admin.columns:
                        merged_admin['시도명'] = merged_admin['시도명_map']
                    elif '시도명_map' in merged_admin.columns:
                        merged_admin['시도명'] = merged_admin['시도명'].fillna(merged_admin['시도명_map'])

                    if '시군구명' not in merged_admin.columns and '시군구명_map' in merged_admin.columns:
                        merged_admin['시군구명'] = merged_admin['시군구명_map']
                    # 누락 통계
                    missing_sido = merged_admin['시도명'].isna().sum() if '시도명' in merged_admin.columns else -1
                    missing_sigungu = merged_admin['시군구명'].isna().sum() if '시군구명' in merged_admin.columns else -1
                    logging.info("[Fallback] 병합 후 누락 시도명: %s, 누락 시군구명: %s", missing_sido, missing_sigungu)
                    merged_admin['시도명'] = merged_admin.get('시도명', '').fillna('')
                    merged_admin['시군구명'] = merged_admin.get('시군구명', '').fillna('')
                    merged_admin['시도_시군구'] = (
                        merged_admin['시도명'].astype(str) + '_' + merged_admin['시군구명'].astype(str)
                    ).str.strip('_')
                    logging.info("[Fallback] 시도명 dtype: %s, 시군구명 dtype: %s", merged_admin['시도명'].dtype, merged_admin['시군구명'].dtype)
                    logging.info("[Fallback] 시도_시군구 샘플: %s", merged_admin['시도_시군구'].head(5).tolist())
                    gini_calculator = calculate_gini.GiniCalculator(merged_admin)
                    gini_result = gini_calculator.calculate_stats('시도_시군구')
                    if gini_result is None:
                        logging.error("지니계수 계산 결과가 None입니다 (Fallback)")
                        return None
                    result = {
                        'raw_data': raw_data,
                        ...
                        'merged_admin': merged_admin,
                        'code_admin': pd.DataFrame(),
                        'merged_district': merged_admin,
                        'bdong_gini': gini_result['grouped']
                    }
                    logging.info(f"{election_name} 데이터 처리 완료 (시군구 Fallback)")
                    return result
                except Exception as fe:
                    logging.error(f"시군구 Fallback 처리 중 오류: {str(fe)}")
                    raise
            ...
            # '행정동'과 '읍면동'은 동일 취지로 처리하며, 전월세의 경우 '법정동'도 허용
            valid_units = ["시군구", "읍면동", "행정동", "선거구", "법정동"]
        ...
        with pd.ExcelWriter(file_path) as writer:
            ...
            if region_unit == '선거구':
                ...
                try:
                    ...
                    logging.warning(f"누락 선거구 시트 생성 중 경고: {str(e)}")
            else:
                # 비-선거구 단위 결과 시트
                safe_sheet = f'{region_unit}_별_지니계수'
                result['bdong_gini'].head(5000).to_excel(writer, sheet_name=safe_sheet, index=False)
