...
from source import load_data, preprocess, calculate_gini, election_processor

logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')


def _process_lease_election(election_data: dict, election_name: str, election_date: str, region_unit: str) -> dict | None:
    try:
        logging.info("[LEASE] 처리 시작 - 선거: %s, 날짜: %s, 지역단위: %s", election_name, election_date, region_unit)

        if election_name not in election_data:
            logging.error("[LEASE] 선거 데이터를 찾을 수 없음: %s", election_name)
            return None

        raw_df = election_data[election_name]
        logging.info("[LEASE] RAW 데이터: %s", raw_df.shape)

        # 전처리 (전월세 전용 로직 내장)
        proc = preprocess.DataProcessor(raw_df)
        data = proc.preprocessing()
        logging.info("[LEASE] 전처리 후: %s", data.shape)
        ...
        if region_unit == '시군구':
            # 지역코드 → 시군구명/시도명 매핑
            lease_df['지역코드'] = lease_df['지역코드'].astype(str).str.zfill(5)
            code_bdong = pdr.code_bdong()[["시군구코드", "시군구명", "시도명"]].drop_duplicates("시군구코드")
            code_bdong['시군구코드'] = code_bdong['시군구코드'].astype(str).str.zfill(5)
            merged = lease_df.merge(code_bdong, how='left', left_on='지역코드', right_on='시군구코드')
            missing = merged['시군구명'].isna().sum()
            logging.info("[LEASE] 시군구 매핑 누락 건수: %d", missing)
            merged['시도_시군구'] = (merged['시도명'].fillna('') + '_' + merged['시군구명'].fillna('')).str.strip('_')
            gini_calc = calculate_gini.GiniCalculator(merged)
            gini_result = gini_calc.calculate_stats('시도_시군구')
        elif region_unit == '법정동':
            # 법정동명 기준 집계 (단일 문자열)
            if '법정동' not in lease_df.columns:
                logging.error("[LEASE] 법정동 컬럼이 존재하지 않습니다")
                return None
            lease_df['법정동'] = lease_df['법정동'].astype(str).str.strip()
            gini_calc = calculate_gini.GiniCalculator(lease_df)
            gini_result = gini_calc.calculate_stats('법정동')
        else:
            logging.error("[LEASE] 지원하지 않는 지역단위: %s", region_unit)
            return None
        ...
        result = {
            ...
            'merged_admin': lease_df,
            'code_election_day': pd.DataFrame(),
            'code_current': pd.DataFrame(),
            'mapping_df': pd.DataFrame(),
            'code_district': pd.DataFrame(),
            'merged_district': pd.DataFrame(),
            'code_admin': pd.DataFrame(),
        }
        logging.info("[LEASE] 처리 완료 - %s", election_name)
        return result

    except Exception as e:
        logging.error("[LEASE] 처리 중 오류: %s", str(e))
        raise
...
def _save_results_lease(results: dict, election_name: str, region_unit: str, directory: str, start_date=None, end_date=None):
    import datetime
    ...
    file_path = os.path.join(directory, f'{fmt(start_date)}_{fmt(end_date)}_{election_name}_{region_unit}_지니계수.xlsx')
    logging.info(f"[LEASE] 결과 저장 시작: {file_path}")

    with pd.ExcelWriter(file_path) as writer:
        res = results[election_name]
        res['raw_data'].head(5000).to_excel(writer, sheet_name='원본', index=False)
        sheet = '시군구별_지니계수' if region_unit == '시군구' else '법정동별_지니계수'
        res['bdong_gini'].head(5000).to_excel(writer, sheet_name=sheet, index=False)
    logging.info(f"[LEASE] 결과 저장 완료: {file_path}")


def process_and_save_all_elections(
    election_list: dict,
    db_path: str,
    table_name: str,
    start_date=None,
    ...
    try:
        logging.info("데이터 처리 시작 - 선거: %s, 기간: %s ~ %s", list(election_list.keys()), start_date, end_date)
        ...
        for election_name, election_date in election_list.items():
            logging.info("%s 처리 시작...", election_name)

            if market_type == '매매':
                # 기존 매매 프로세서 위임
                logging.info("[SALE] 기존 매매 프로세스 위임")
                sale_results = election_processor.process_and_save_all_elections(
                    {election_name: election_date}, db_path, table_name, start_date, end_date, region_unit
                )
                if sale_results and election_name in sale_results:
                    results[election_name] = sale_results[election_name]
                continue

            # 전월세 경로
            result = _process_lease_election(election_data, election_name, election_date, region_unit)
            if result is None:
                logging.error("%s 처리 실패", election_name)
                continue

            results[election_name] = result
            logging.info("%s 처리 완료", election_name)
            logging.info("%s 저장 시작...", election_name)
            _save_results_lease(results, election_name, region_unit, folder, start_date, end_date)

        logging.info("모든 선거 데이터 처리 및 저장 완료")
        return results

    except Exception as e:
        logging.error("데이터 처리 중 오류 발생: %s", str(e))
        raise
